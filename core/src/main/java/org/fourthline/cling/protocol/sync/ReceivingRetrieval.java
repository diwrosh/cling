...
import org.teleal.cling.model.resource.InputStreamResource;
...
public class ReceivingRetrieval extends ReceivingSync<StreamRequestMessage, StreamResponseMessage> {
	...
	protected StreamResponseMessage executeSync() {
		...
		if (foundResource == null) {
			...
			ResourceProcessor processor =  getUpnpService().getRegistry().getResourceMissingProcessor();
			if (processor == null) {
				return null;
			}
			
			try {
				foundResource = processor.process(requestedURI);
				if (foundResource == null) {
					return null;
				}
			} catch (Exception e) {
				//log.warning("could not process resource: " + requestedURI + ": " + e);
				return null;
			}
		}
	...
	protected StreamResponseMessage createResponse(URI requestedURI, Resource resource) {
		...
		try {
			...
			} else if (InputStreamResource.class.isAssignableFrom(resource.getClass())) { 

				InputStreamResource inputStreamResource = (InputStreamResource) resource;

				try {
					response = new StreamResponseMessage(inputStreamResource.getInputStream(), inputStreamResource.getMimeType(), inputStreamResource.getSize());
				} catch (Exception e) {
					log.warning("Cannot find resource: " + inputStreamResource.getModel());
