...
public class PositionInfo
{
  ...
  public PositionInfo(Map<String, ActionArgumentValue> args) {
    // 1.2.5 changes java.lang.ClassCastException; java.lang.NullPointerException
    int argRelCount = Integer.MAX_VALUE;
    int argAbsCount = Integer.MAX_VALUE;

    try {
      argRelCount = (Integer) args.get("RelCount").getValue();
    } catch (Exception ex) {
    }

    try {
      argAbsCount = (Integer) args.get("AbsCount").getValue();
    } catch (Exception ex) {
    }

    initialize(((UnsignedIntegerFourBytes) args.get("Track").getValue()).getValue(), (String) args.get("TrackDuration").getValue(),
        (String) args.get("TrackMetaData").getValue(), (String) args.get("TrackURI").getValue(), (String) args.get("RelTime").getValue(),
        (String) args.get("AbsTime").getValue(), argRelCount, argAbsCount);
  }
  ...
  public PositionInfo(long track, String trackDuration, String trackMetaData, String trackURI, String relTime, String absTime,
      int relCount, int absCount) {
    initialize(track, trackDuration, trackMetaData, trackURI, relTime, absTime, relCount, absCount);
  }

  private void initialize(long track, String trackDuration, String trackMetaData, String trackURI, String relTime, String absTime,
      int relCount, int absCount) {
    this.track = new UnsignedIntegerFourBytes(track);
    this.trackDuration = trackDuration;
  ...
  public String toString() {
    return "(PositionInfo) Track: " + getTrack() + " RelTime: " + getRelTime() + " Duration: " + getTrackDuration() + " Percent: "
        + getElapsedPercent();
  }
}
