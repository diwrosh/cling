...
public abstract class Browse extends ActionCallback
{
  ...
  /**
   * @param maxResults
   *          Can be <code>null</code>, then {@link #getDefaultMaxResults()} is used.
   */
  public Browse(Service service, String objectID, BrowseFlag flag, String filter, long firstResult, Long maxResults, SortCriterion... orderBy) {
    ...
    getActionInvocation().setInput("RequestedCount", new UnsignedIntegerFourBytes(maxResults == null ? getDefaultMaxResults() : maxResults));
  ...
  public void success(ActionInvocation invocation) {
    ...
    // (v10)/(V14) Null pointer exception on browse success with empty result
    Object resVal = (null == invocation.getOutput("Result")) ? null : invocation.getOutput("Result").getValue();
    String resStr = (null == resVal) ? "" : resVal.toString();

    BrowseResult result = new BrowseResult(resStr, (UnsignedIntegerFourBytes) invocation.getOutput("NumberReturned").getValue(), (UnsignedIntegerFourBytes) invocation.getOutput("TotalMatches").getValue(), (UnsignedIntegerFourBytes) invocation.getOutput("UpdateID").getValue());
  ...
  public boolean receivedRaw(ActionInvocation actionInvocation, BrowseResult browseResult) {
    /*
     * if (log.isLoggable(Level.FINER)) { log.finer("-------------------------------------------------------------------------------------"); log.finer("\n" + XML.pretty(browseResult.getDidl())); log.finer("-------------------------------------------------------------------------------------"); }
     */
    return true;
  }
